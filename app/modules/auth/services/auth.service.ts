...
export class AuthService {
  ...
  private generateToken(user: User): string {
    const payload: JWTPayload = {
      id: user.id,
      email: user.email || '', // Utiliser une chaîne vide comme fallback
      name: user.name || '',   // Ajout du name avec fallback
      role: user.role,
      isFirstLogin: user.isFirstLogin || false // Ajout de isFirstLogin avec fallback
    };
  ...
  async login(credentials: LoginUserDTO): Promise<AuthResponse | AuthError> {
    try {
      ...
      // Cela sera fait dans le composant de connexion
  ...
  async register(data: RegisterUserDTO): Promise<{ message: string, success: boolean, user: Pick<User, 'id'> } | AuthError> {
    try {
      ...
      return {
        ...
        success: true,
        user: { id: user.id } // Retourner uniquement l'ID de l'utilisateur
    };
    } catch (error: unknown) {
